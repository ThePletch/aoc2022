...
(defn perform-instruction [stacks instruction]
  (if (<= (get instruction :quantity) 0)
      ...
      (let [src (get instruction :src)
            ...
            src-top (take quantity (get stacks src))]
        (assoc (assoc stacks src (nthrest (get stacks src) quantity))
               dest
               ; append the elements in order (reverse, then append in reverse order, because clojure)
               (apply conj (get stacks dest) (reverse src-top))))))
